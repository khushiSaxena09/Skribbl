...
export default function Game({ roomData }) {
  ...
  useEffect(() => {
    const onRoundStart = (data) => {
      ...
      setWordOptions(amIDrawer && data.words ? [...data.words] : []);
      setTimeLeft(data.timeLeft || 60);
      setCurrentRound(data.round || 1);
      setTotalRounds(data.totalRounds || 3);
      ...
      if (amIDrawer) {
        addNotification(`✏️ Your turn! Pick a word`, 'game');
      }
    };

    const onTimer = ({ timeLeft }) => {
      setTimeLeft(timeLeft);
      if (timeLeft === 10) addNotification('⏰ Hurry! 10s left', 'warning');
    };

    const onWordChosen = ({ length }) => {
      setWordLength(length);
      setWordChosen(true);
      setWordOptions([]);
    };
    ...
    const onCorrectGuess = ({ playerId, playerName, points }) => {
      setPlayers((prev) =>
        prev.map((p) =>
    ...
    const onGameOver = ({ winner, winnerScore, players }) => {
      setGameEnded(true);
      setWinner({ name: winner, score: winnerScore });
      setPlayers(players);
    };
  ...
  if (gameEnded) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-indigo-900 via-purple-900 to-slate-900 flex items-center justify-center p-6">
        <div className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-2xl p-10 text-center text-white shadow-2xl">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-950 via-slate-900 to-purple-950 p-4 text-white">

      {/* Top HUD */}
      <div className="grid grid-cols-4 gap-3 mb-4">
        {[
          ['Round', `${currentRound}/${totalRounds}`],
          ['Drawer', isDrawer ? 'You' : players.find(p => p.id === drawerId)?.name || '...'],
          ['Time', `${timeLeft}s`],
          ['Word', wordLength ? `${wordLength}` : '?']
        ].map(([label, value]) => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
        ...
        <div className="lg:col-span-3 space-y-3">
          ...
          {wordOptions.length > 0 && isDrawer && (
            <div className="bg-yellow-500/20 border border-yellow-400 rounded-xl p-4 text-center">
              <h2 className="font-bold mb-2">Choose a word</h2>
              <div className="flex gap-2 justify-center flex-wrap">
                {wordOptions.map((w, i) => (
                  <Button key={i} onClick={() => chooseWord(w)}>
                    {w}
                  </Button>
                ))}
              </div>
          ...
          <DrawingCanvas
            ...
            brushSize={brushSize}
            onColorChange={(e) => setColor(e.target.value)}
            onBrushSizeChange={(e) => setBrushSize(+e.target.value)}
            wordLength={wordLength}
            timeLeft={timeLeft}
            totalTime={60}
            roomId={roomData.roomId}
        ...
        {/* Sidebar */}
        <div className="space-y-4">
          <PlayerList players={players} playerId={roomData.playerId} />
          <ChatBox
            messages={chatMessages}
            onSendMessage={(text) =>
              socket.emit('room_event', {
